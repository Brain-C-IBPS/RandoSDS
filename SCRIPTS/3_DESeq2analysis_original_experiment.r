...
#   perform DESeq2 analysis and DEG extraction for original experiment
#
#-----------------------------------------------------------------------------
#
...
pre_processing_dir <- file.path(paste0(parent_dir, '/ANALYSIS_RESULTS/PRE_PROCESSING_ORIGINAL_EXPERIMENT'))
print(pre_processing_dir)
setwd(pre_processing_dir)

# List of polyQ lengths in experiment
polyQ <- c("Q20", "Q50", "Q111", "Q170", "Q175")

# Get count file
dat <- as.matrix(read.table("count_dat_norm_All.txt",header = T))
...
out.dirB <- paste0(getwd(), "/DESeq2_RESULTS") 
dir.create(out.dirB)
setwd(out.dirB)
...
for(i in 1:1){  
  ...
  for( j in 2:5 ) {
    ...
    #-- Define a DESeq dataset obeject --#
    ...
    png(filename = paste(out.dir, paste("Boxplots_pseudoreadcounts_",polyQ[j],".png",sep=""), sep=""), width = 2000, height = 2500, units = "px", pointsize = 12, bg = "white",  res=300)
    ...
    # Apparently we store all LFC values, without filtering based on p-values
    lFC <- as.matrix(res$log2FoldChange)
    rownames(lFC) <- rownames(res) 
    
    png(filename = paste(out.dir, paste("VolcanoPlot_DESeq2_",polyQ[j],"_",".png",sep=""), sep=""), width = 2000, height = 2500, units = "px", pointsize = 12, bg = "white",  res=300)
    par(las=1)
    print('Volcano plot')
...
for(i in 1:1){  
  ...
  write.table(LFC.cible, "Table_LFC.txt" ,row.names = T,col.names = T, quote = F,sep = "\t")
  write.table(Pval.cible, "Table_Pval.txt" ,row.names = T,col.names = T, quote = F,sep = "\t")
