...
-----
    ...
    LFC files of the BS replicates of the experiment containing ENSEMBL gene IDs on 
    the first column and LFC values on second colum
    
    By default, input LFC files are taken from the /DESEq2_RESULTS/ folder within the 
    /ANALYSIS_RESULTS/PRE_PROCESSING_BS_REPLICATES/BSreplicate_*/ directory, where the
    wildcard corresponds to numbers in the range of the number of BS replicates that
    are generated on the first step of the Rando-SDS workflow. LFC files have a naming
...
------
    ...
    on the first column and corresponding LFC values on the second column. 
...
# Set path to folders containing LFC files of BS replicates of the experiment;
lfc_files_path_string = './ANALYSIS_RESULTS/PRE_PROCESSING_BS_REPLICATES/BSreplicate_*'

# List directories containing BS replicates of the experiment
number_of_bs_replicates_to_evaluate = 0
list_of_bs_folders = []

for file in glob.glob(lfc_files_path_string):
    number_of_bs_replicates_to_evaluate += 1
    list_of_bs_folders.append(file)
    
# Iterate over BS files:
for bs_folder in list_of_bs_folders:
    
    print(' --------------------------------------------------------')
    print('     CHANGING GENE NAMES OF LFC FILES IN ', Path(bs_folder).stem)  
    print(' --------------------------------------------------------')
    
    # Retrieve LFC files for which we want to change ENSEMBL IDs to GeneSymbols:
    lfc_files_to_modify = []
        
    for file in glob.glob(''.join([bs_folder, '/DESEq2_RESULTS/lfc_*.txt'])):
        lfc_files_to_modify.append(file)
    
    # Loop over files and query mygene library for translation:
    for file in lfc_files_to_modify: 
    
        # Read as dataframe and modify column names
        lfc_df = pd.read_csv( file, header = None, sep = '\t')
        ...
        new_lfc_df = new_lfc_df[ new_lfc_df['lfc'] != 0.]
        
        # Save df with gene symbols instead of ensembl ids to txt file
        outdir = os.path.dirname(file)
        old_file_name = Path(file).stem
